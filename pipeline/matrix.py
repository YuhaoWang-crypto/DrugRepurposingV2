...
def align_expr_and_meta(
    ...
    **_kwargs,
    ...
    def clean_id(x) -> str:
        """Normalize sample identifiers for fuzzy matching.

        GEO data is messy: expression matrices may label columns as GSM IDs,
        or as derived IDs like "XGSM12345", "GSM12345.1", etc. We:
        1) Extract GSM\d+ if present (preferred)
        2) Else, keep an uppercase alphanumeric token (remove separators)

        This is only used for *matching*, not for display.
        """
        ...
        if not s:
            return ""
        s_up = s.upper()
        m = re.search(r"(GSM\d+)", s_up)
        if m:
            return m.group(1)
        # drop separators like '-', '_', spaces, '.', ':', etc.
        s_up = re.sub(r"[^A-Z0-9]+", "", s_up)
        return s_up

    # Strategy 0: if expression column names *contain* GSM IDs (e.g. "XGSM123"
    # or "GSM123_patient1"), extract and normalize them.
    gsm_extract_map: Dict[str, str] = {}
    for c in df_expr.columns:
        s = str(c)
        m = re.search(r"(GSM\d+)", s, flags=re.IGNORECASE)
        if m:
            gsm_extract_map[c] = m.group(1).upper()
    if gsm_extract_map:
        df_expr = df_expr.rename(columns=gsm_extract_map)

    # Strategy 1: direct intersection
    common = [c for c in df_expr.columns if c in df_meta.index]

    # Strategy 1b: cleaned-id intersection (handles XGSM123, GSM123.1, etc.)
    if not common:
        meta_clean_to_gsm = {clean_id(gsm): gsm for gsm in df_meta.index}
        rename_clean: Dict[str, str] = {}
        for c in df_expr.columns:
            cc = clean_id(c)
            if cc in meta_clean_to_gsm:
                rename_clean[c] = meta_clean_to_gsm[cc]
        if rename_clean:
            df_expr = df_expr.rename(columns=rename_clean)
        common = [c for c in df_expr.columns if c in df_meta.index]
