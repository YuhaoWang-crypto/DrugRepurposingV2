...
def bh_fdr(pvals: np.ndarray) -> np.ndarray:
    ...
    Returns adjusted p-values (q-values) with the same shape as `pvals`.
    NaNs are preserved.
    """
...
def compute_de_bulk_ttest(df_expr: pd.DataFrame, meta: pd.DataFrame, eps: float = 1e-6) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        ...
        'pval': pvals,
        'padj': padj,
        'abs_log2fc': np.abs(lfc),
    })
    df = df.sort_values(['padj', 'pval'], ascending=True).reset_index(drop=True)
    return df
