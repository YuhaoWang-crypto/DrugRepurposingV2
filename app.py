...
def _parse_terms_text(text: str):
    """Parse a textarea into a list of terms.

    Supports comma- or newline-separated input. Preserves order and de-dupes
    case-insensitively.
    """
    if not text:
        return []

    # allow commas and newlines
    parts = []
...
def _init_state():
    ...
    # Sidebar helper fields (Case/Control keywords). We keep them outside the JSON editor
    # so users can tweak labeling quickly without editing raw JSON.
    cond0 = (st.session_state["config"] or {}).get("condition", {})
    if "case_terms_ui" not in st.session_state:
        st.session_state["case_terms_ui"] = "\n".join(cond0.get("case_terms", []) or [])
    if "control_terms_ui" not in st.session_state:
        st.session_state["control_terms_ui"] = "\n".join(cond0.get("control_terms", []) or [])
    if "prefer_keys_ui" not in st.session_state:
        st.session_state["prefer_keys_ui"] = "\n".join(cond0.get("prefer_keys", []) or [])
    if "ambiguous_policy_ui" not in st.session_state:
        st.session_state["ambiguous_policy_ui"] = (cond0.get("ambiguous_policy") or "ambiguous")
...
def _reset_config_callback():
    ...
    # Keep the helper labeling widgets in sync with the applied config.
    cond = cfg0.get("condition", {}) if isinstance(cfg0, dict) else {}
    st.session_state["case_terms_ui"] = "\n".join(cond.get("case_terms", []))
    st.session_state["control_terms_ui"] = "\n".join(cond.get("control_terms", []))
    st.session_state["prefer_keys_ui"] = "\n".join(cond.get("prefer_keys", []))
    st.session_state["ambiguous_policy_ui"] = cond.get("ambiguous_policy", "ambiguous")
...
def _apply_config_callback():
    try:
        ...
        # Sync helper labeling widgets
        cond = cfg_merged.get("condition", {}) if isinstance(cfg_merged, dict) else {}
        st.session_state["case_terms_ui"] = "\n".join(cond.get("case_terms", []))
        st.session_state["control_terms_ui"] = "\n".join(cond.get("control_terms", []))
        st.session_state["prefer_keys_ui"] = "\n".join(cond.get("prefer_keys", []))
        st.session_state["ambiguous_policy_ui"] = cond.get("ambiguous_policy", "ambiguous")
...
def _apply_labeling_callback():
    """Apply Case/Control keyword widgets into config JSON (condition.*)."""
    try:
        cfg = st.session_state.get("config")
        if not isinstance(cfg, dict):
            cfg = default_config()

        case_terms = _parse_terms_text(st.session_state.get("case_terms_ui", ""))
        control_terms = _parse_terms_text(st.session_state.get("control_terms_ui", ""))
        prefer_keys = _parse_terms_text(st.session_state.get("prefer_keys_ui", ""))
        ambiguous_policy = st.session_state.get("ambiguous_policy_ui", "ambiguous")
        ...
        st.session_state["_cfg_editor_pending"] = json.dumps(cfg2, ensure_ascii=False, indent=2)
        st.session_state.pop("_cfg_error", None)
        add_log("[UI] Updated case/control keywords from sidebar")
    except Exception as e:
        st.session_state["_cfg_error"] = str(e)
        add_log(f"[UI] Failed to update case/control keywords: {e}")


st.sidebar.markdown("#### Case/Control labeling")
st.sidebar.caption(
    "Used to infer **case / control** from GEO sample metadata (characteristics). "
    "Edit here and click **Apply case/control keywords**."
)

st.sidebar.text_area(
    "Case keywords (one per line or comma-separated)",
    key="case_terms_ui",
    height=110,
)
st.sidebar.text_area(
    "Control keywords (one per line or comma-separated)",
    key="control_terms_ui",
    height=110,
)
st.sidebar.text_area(
    "Prefer metadata keys (optional, one per line)",
    key="prefer_keys_ui",
    height=90,
)
st.sidebar.selectbox(
    "If both case & control match",
    options=["ambiguous", "case", "control"],
    key="ambiguous_policy_ui",
)
st.sidebar.button(
    "Apply case/control keywords",
    use_container_width=True,
    on_click=_apply_labeling_callback,
)
...
with val_tab:
    ...
    if df_cand is None or df_cand.empty:
        ...
        if st.button("Validate selected GSE"):
            ...
            for gse in selected:
                try:
                    ...
                    # Keep a small in-memory copy for previewing characteristics / tweaking label rules.
                    # (Stored only for the last validation run.)
                    meta_store[gse] = meta
            ...
            st.session_state["validation_meta"] = meta_store
        ...
        if df_val is not None and not df_val.empty:
            ...
            # ---- Characteristics preview (to help refine case/control terms) ----
            meta_store = st.session_state.get("validation_meta", {}) or {}
            if meta_store:
                with st.expander("Preview sample characteristics (to refine case/control rules)", expanded=False):
                    gse_prev = st.selectbox(
                        "Choose a validated GSE to preview",
                        options=[g for g in gse_run_options if g in meta_store] if "gse_run_options" in locals() else list(meta_store.keys()),
                        key="char_prev_gse",
                    )
                    meta_prev = meta_store.get(gse_prev)
                    if meta_prev is not None and not meta_prev.empty:
                        st.caption(
                            "Tip: Look at the **characteristics** and **characteristics_kv** fields to decide what keywords belong in "
                            "sidebar → *Case/Control labeling*."
                        )

                        # Sample-level view
                        cols = [c for c in ["sample_id", "condition", "platform_id", "title", "source", "characteristics"] if c in meta_prev.columns]
                        st.markdown("**Sample-level preview**")
                        st.dataframe(meta_prev[cols].head(50), use_container_width=True)

                        # Key/value summary
                        if "characteristics_kv" in meta_prev.columns:
                            from collections import Counter, defaultdict

                            key_counts = Counter()
                            val_counts = defaultdict(Counter)
                            for kv in meta_prev["characteristics_kv"].tolist():
                                if isinstance(kv, dict):
                                    for k, v in kv.items():
                                        ...
                                        key_counts[str(k)] += 1
                                        if v is not None:
                                            val_counts[str(k)][str(v)] += 1

                            df_keys = pd.DataFrame(
                                [
                                    {
                                        "key": k,
                                        "n_samples_with_key": int(key_counts[k]),
                                        "top_values": "; ".join([f"{vv} ({cc})" for vv, cc in val_counts[k].most_common(5)]),
                                    }
                                    for k in key_counts
                                ]
                            ).sort_values("n_samples_with_key", ascending=False)

                            st.markdown("**Top characteristics keys / values**")
                            st.dataframe(df_keys.head(40), use_container_width=True)

                        st.download_button(
                            f"Download {gse_prev} labeled sample meta CSV",
                            meta_prev.to_csv(index=False).encode("utf-8"),
                            file_name=f"{gse_prev}_sample_meta_labeled.csv",
                        )
...
with res_tab:
    ...
    if not results:
        ...
        st.markdown("### Differential expression details")
        de_map = {}
        for r in results:
            gse = r.get("gse")
            de_csv = r.get("de_csv")
            if gse and de_csv and os.path.exists(de_csv):
                de_map[gse] = de_csv

        if not de_map:
            st.info("No DE tables found. (Expected: <project_dir>/proc/<GSE>/DE_bulk_case_vs_control.csv)")
        else:
            gse_de = st.selectbox("Select GSE to view DE", list(de_map.keys()), key="de_gse_select")
            de_path = de_map[gse_de]
            try:
                df_de = pd.read_csv(de_path)
            except Exception as e:
                st.error(f"Failed to read DE table: {de_path}\n{repr(e)}")
                df_de = None

            if df_de is not None and not df_de.empty:
                st.caption(f"DE table saved at: {de_path}")

                # Small sanity/help note: many GEO microarrays use probe/transcript-cluster IDs.
                if "gene_symbol" not in df_de.columns and "gene_raw" in df_de.columns:
                    st.info(
                        "**Note on gene IDs**: this DE table currently shows `gene_raw` as it appears in the GEO expression matrix. "
                        "For many microarray platforms, these are **probe / transcript-cluster IDs** (e.g., `TC...hg.1`) rather than HGNC gene symbols. "
                        "This is expected — mapping to gene symbols requires platform annotation (GPL)."
                    )

                # Basic filters/sorting
                col_f1, col_f2, col_f3 = st.columns([2, 2, 1])
                with col_f1:
                    gene_filter = st.text_input("Filter genes (contains)", value="", key="de_gene_filter")
                with col_f2:
                ...
                with col_f3:
                    top_n = st.number_input("Rows", min_value=10, max_value=10000, value=200, step=50, key="de_top_n")

                df_view = df_de.copy()
                if gene_filter:
                    # Try gene_symbol first if present, otherwise gene_raw.
                    gene_col = "gene_symbol" if "gene_symbol" in df_view.columns else ("gene_raw" if "gene_raw" in df_view.columns else None)
                    if gene_col:
                        df_view = df_view[df_view[gene_col].astype(str).str.contains(gene_filter, case=False, na=False)]

                # Sorting
                if sort_by == "padj (asc)" and "padj" in df_view.columns:
                    df_view = df_view.sort_values(["padj", "pval"], ascending=[True, True])
                elif sort_by == "pval (asc)" and "pval" in df_view.columns:
                    df_view = df_view.sort_values("pval", ascending=True)
                elif sort_by == "|log2fc| (desc)" and "log2fc" in df_view.columns:
                    df_view = df_view.assign(abs_log2fc=np.abs(df_view["log2fc"]))
                    df_view = df_view.sort_values("abs_log2fc", ascending=False)
                elif sort_by == "log2fc (desc)" and "log2fc" in df_view.columns:
                    df_view = df_view.sort_values("log2fc", ascending=False)
                elif sort_by == "log2fc (asc)" and "log2fc" in df_view.columns:
                    df_view = df_view.sort_values("log2fc", ascending=True)

                st.dataframe(df_view.head(int(top_n)), use_container_width=True, height=420)
                st.download_button(
                    f"Download {gse_de} DE CSV",
                    df_de.to_csv(index=False).encode("utf-8"),
                    file_name=f"{gse_de}_DE_bulk_case_vs_control.csv",
                )

                # Top up/down quick view
                if "log2fc" in df_de.columns:
                    st.markdown("#### Top genes (quick view)")
                    col_u, col_d = st.columns(2)
                    with col_u:
                        st.caption("Top UP (highest log2fc)")
                        st.dataframe(df_de.sort_values("log2fc", ascending=False).head(25), use_container_width=True, height=350)
                    with col_d:
                        st.caption("Top DOWN (lowest log2fc)")
                        st.dataframe(df_de.sort_values("log2fc", ascending=True).head(25), use_container_width=True, height=350)
